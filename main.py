 ...
 #recorrer lista de clientes
    for i, obj_cliente in enumerate(lista_clientes):
        if obj_cliente.id == cliente_id:
            return obj_cliente
    raise HTTPException(
          status_code=status, detail=f"El cliete con id {cliente_id}, no existe."
    )
...
async def crear_factura(cliente_id: int, datos_factura: FacturaCrear):
  ...
  for cliente in lista_clientes:
      ...
      #mensaje si no existe un cliente
...
async def crear_transacciones(factura_id: int, datos_transaccion: TransaccionCrear):
  ...
  for factura in lista_facturas:
      ...
      #mensaje si no existe una factura
...
#validar datos de la transaccion
  ...
  factura_encontrada.transsacciones.append(transaccion_val)
  #id de la factura
  transaccion_val.id = len(lista_trasscciones) + 1
